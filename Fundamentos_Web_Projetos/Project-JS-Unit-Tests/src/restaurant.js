...
function payment() {
  let products = this.fetchMenu();
  let finalPrice = 0;
  for (let product of this.comsumption) {
    if (products.food[product]) {
      finalPrice += (products.food[product]);
    }
    if (products.drinks[product]) {
      finalPrice += (products.drinks[product]);
    }
  }
  return finalPrice;
}
